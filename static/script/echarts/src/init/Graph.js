...
define(function(require) {
     ...
     *  5| x  x  x  x  x
     * ```
     * 节点的行列总和会被写到`node.init.value`
     * 对于有向图会计算每一行的和写到`node.init.outValue`,
     * 计算每一列的和写到`node.init.inValue`。
     * 边的权重会被然后写到`edge.init.weight`。
     * 
     * @method module:echarts/init/Graph.fromMatrix
     * @param {Array.<Object>} nodesData 节点信息，必须有`id`属性, 会保存到`node.init`中
